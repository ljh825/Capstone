 ...
 *
 * This file contains the implementation in C++ of a INDI telescope driver using the Radio Telescope API.
...
// We declare an auto pointer to radioTelescope.
std::unique_ptr<radioTelescope> radioTelescopePtr(new radioTelescope());
...
{
    radioTelescopePtr->ISGetProperties(dev);
}

void ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    radioTelescopePtr->ISNewSwitch(dev, name, states, names, n);
}

void ISNewText(const char *dev, const char *name, char *texts[], char *names[], int n)
{
    radioTelescopePtr->ISNewText(dev, name, texts, names, n);
}

void ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    radioTelescopePtr->ISNewNumber(dev, name, values, names, n);
}

void ISNewBLOB(const char *dev, const char *name, int sizes[], int blobsizes[], char *blobs[], char *formats[],
               char *names[], int n)
{
    radioTelescopePtr->ISNewBLOB(dev, name, sizes, blobsizes, blobs, formats, names, n);
}

void ISSnoopDevice(XMLEle *root)
{
    radioTelescopePtr->ISSnoopDevice(root);
}

radioTelescope::radioTelescope() : TrackLogFileName(GetHomeDirectory() + "/.indi/sw_mount_track_log.txt")
{
    // Set up the logging pointer in radioTelescopeAPI
...
bool radioTelescope::Abort()
{
    DEBUG(DBG_SCOPE, "radioTelescope::Abort");
...
bool radioTelescope::Handshake()
{
    DEBUG(DBG_SCOPE, "radioTelescope::Handshake");
    ...
    DEBUGF(DBG_SCOPE, "radioTelescope::Handshake - Result: %d", Result);
    return Result;
}

const char *radioTelescope::getDefaultName()
{
    //DEBUG(DBG_SCOPE, "radioTelescope::getDefaultName\n");
    return "Radio Telescope Dish";
}

bool radioTelescope::Goto(double ra, double dec)
{
    DEBUG(DBG_SCOPE, "radioTelescope::Goto");
    ...
    /* TODO: Replace this with logic for our dish.
    ...
    */
    ...
    /* TODO: Find a workable replacement to this.
    ...
    */
    ...
    /* TODO: Determine how to work this to our system.
    ...
    */
    ...
    /* TODO: Replace this with values for our system.
...
bool radioTelescope::initProperties()
{
    IDLog("radioTelescope::initProperties\n");
    ...
    /* TODO: Figure out what we will need for our telescope dish.
...
void radioTelescope::ISGetProperties(const char *dev)
{
    IDLog("radioTelescope::ISGetProperties\n");
...
bool radioTelescope::ISNewBLOB(const char *dev, const char *name, int sizes[], int blobsizes[], char *blobs[],
...
bool radioTelescope::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
...
bool radioTelescope::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
...
bool radioTelescope::ISNewText(const char *dev, const char *name, char *texts[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        // It is for us
...
void radioTelescope::UpdateScopeConfigSwitch()
{
...
double radioTelescope::GetSlewRate()
{
...
bool radioTelescope::MoveNS(INDI_DIR_NS dir, TelescopeMotionCommand command)
{
    DEBUG(DBG_SCOPE, "radioTelescope::MoveNS");
...
bool radioTelescope::MoveWE(INDI_DIR_WE dir, TelescopeMotionCommand command)
{
    DEBUG(DBG_SCOPE, "radioTelescope::MoveWE");
...
double radioTelescope::GetParkDeltaAz(ParkDirection_t target_direction, ParkPosition_t target_position)
{
    double Result = 0;

    DEBUGF(DBG_SCOPE, "GetParkDeltaAz: direction %d - position: %d", (int)target_direction, (int)target_position);
    
    /* TODO: Change this to straight up for our dish
...
bool radioTelescope::Park()
{
    DEBUG(DBG_SCOPE, "radioTelescope::Park");
    ...
    /* TODO: Figure out if we need this, or if it can be hard-coded.
    ...
    */
    ...
    /* TODO: Change this to what we need for our dish.
    ...
    */
    ...
    /* TODO: Change microsteps to something more useful.
    ...
    */
    ...
    /* TODO: Command it with something other than microsteps.
    SlewTo(AXIS1, AzimuthOffsetMicrosteps);
    SlewTo(AXIS2, AltitudeOffsetMicrosteps);
    */
...
bool radioTelescope::UnPark()
{
    DEBUG(DBG_SCOPE, "radioTelescope::UnPark");
    ...
    */
    ...
    /* TODO: Replace with our telescope's equivalent.
    // Move the telescope to the desired position
    long AltitudeOffsetMicrosteps = DegreesToMicrosteps(AXIS2, DeltaAlt);
    long AzimuthOffsetMicrosteps  = DegreesToMicrosteps(AXIS1, DeltaAz);
    */
    ...
    /* TODO: Replace with something more useful for us.
    ...
    */
    ...
    /* TODO: Determine the command our telescope will need.
    SlewTo(AXIS1, AzimuthOffsetMicrosteps);
    SlewTo(AXIS2, AltitudeOffsetMicrosteps);
    */
...
bool radioTelescope::ReadScopeStatus()
{
    //    DEBUG(DBG_SCOPE, "radioTelescope::ReadScopeStatus");

    // leave the following stuff in for the time being it is mostly harmless

    /* TODO: Figure out what we do instead of this, if anything.
    ...
    */
    ...
    /* TODO: Figure out how to make this for our telescope.
...
bool radioTelescope::saveConfigItems(FILE *fp)
{
...
bool radioTelescope::Sync(double ra, double dec)
{
    DEBUG(DBG_SCOPE, "radioTelescope::Sync");
    ...
    switch (TrackState)
    {
        ...
        case SCOPE_TRACKING:
        {
            ...
            else
            {
                ...
                /* TODO: Figure out if this needs to be adapted.
...
bool radioTelescope::updateProperties()
{
...
IPState radioTelescope::GuideNorth(uint32_t ms)
{
    GuidingPulse Pulse;
...
IPState radioTelescope::GuideSouth(uint32_t ms)
{
    GuidingPulse Pulse;
...
IPState radioTelescope::GuideWest(uint32_t ms)
{
    GuidingPulse Pulse;
...
IPState radioTelescope::GuideEast(uint32_t ms)
{
    GuidingPulse Pulse;
...
void radioTelescope::ResetGuidePulses()
{
    GuidingPulses.clear();
}

int radioTelescope::recover_tty_reconnect()
{
...
int radioTelescope::radioTelescope_tty_read(int fd, char *buf, int nbytes, int timeout, int *nbytes_read)
{
...
int radioTelescope::radioTelescope_tty_read_section(int fd, char *buf, char stop_char, int timeout, int *nbytes_read)
{
    if (!recover_tty_reconnect())
    {
        return 0;
...
int radioTelescope::radioTelescope_tty_write(int fd, const char *buffer, int nbytes, int *nbytes_written)
{
    if (!recover_tty_reconnect())
    {
        return 0;
...
void radioTelescope::UpdateDetailedMountInformation(bool InformClient)
{
    /* TODO: Change this all to be specific to our telescope. 
...
ln_hrz_posn radioTelescope::GetAltAzPosition(double ra, double dec, double offset_in_sec)
{
...
ln_equ_posn radioTelescope::GetRaDecPosition(double alt, double az)
{
    ln_lnlat_posn Location { 0, 0 };
    ln_equ_posn Eq { 0, 0 };
    ...
    {
        // TODO: This just sets to the poles? Can it be hard-coded to Saskatoon?
        if (LocationN[LOCATION_LATITUDE].value > 0)
        {
            Location.lat = 90;
            Location.lng = 0;
...
void radioTelescope::LogMessage(const char* format, ...)
{
